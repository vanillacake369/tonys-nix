...
# Optionally triggers peer review via another provider.
{
  ...
  in
    pkgs.writeShellScript "strategy-lint-${name}.sh" ''
      ...
      # Reversibility gate: cognitive guardrails only apply to irreversible work.
      # If the marker file is missing or contains "reversible", skip all checks.
      ...
      # Check required sections (irreversible work only)
      MISSING=()
      ${sectionChecks}
