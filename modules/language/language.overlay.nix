# Pin terraform
# NOTE :
# state file format locks to version;
# accidental upgrade can corrupt state.
_final: prev: {
  ...
  # Pin Java toolchain to zulu21
  # NOTE :
  # jdtls requires Java 21+ (raises Exception if major < 21),
  # and all Java LSP/build tools must share the same JDK
  # to avoid split-runtime conflicts.
  ...
  # Pin Node.js toolchain to nodejs_24
  # NOTE :
  # nixpkgs default is 24.x and all bundled LSP servers
  # ship nodejs-slim-24 internally,
  # aligning the declared version eliminates split-runtime warnings.
  ...
  # Fix python-lsp-server
  # requires jedi<0.20.0 but nixpkgs ships 0.20.0
