# Keymap SSOT: data + inline contract validation (spec merged from keymap-validator.nix).
# Consumers import this module and get back the keybinds attrset. The `keymaps`
# list is self-validated on import: a missing/invalid entry throws a clear error.
# `detectType` and `validate` are also exported so tests can call them directly.
{
  lib,
  userProfile,
}: let
  # ---------------------------------------------------------------------------
  # Inline spec (formerly keymap-validator.nix)
  # ---------------------------------------------------------------------------
  detectType = m:
  ...
  validateEntry = i: m:
    assert m ? bind || throw "keymaps[${toString i}]: missing 'bind'";
    assert m ? tags && builtins.isList m.tags || throw "keymaps[${toString i}] (${m.bind}): missing or invalid 'tags'";
    assert detectType m != "unknown" || throw "keymaps[${toString i}] (${m.bind}): no action field (to/exec/shell/to_if_alone)"; m;
...
in {
  # Exported spec functions (for tests and pipeline consumers)
  inherit detectType validate;

  # Keybinds data — keymaps list is self-validated on import
  workspaces = {
    Docs = {
      monitor = 1;
      apps = ["Obsidian"];
