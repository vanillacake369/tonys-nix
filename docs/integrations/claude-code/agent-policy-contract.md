...
```
lib/agent-policy/
├── agent-contract.nix        # Interface: option type 선언
├── agent-assertions.nix      # Build-time contract validation
├── agent-hook-adapters.nix   # Provider별 hook format adapter (SSoT)
├── agent-assembler.nix       # IoC assembler (entry point)
└── mixins/
...
`lib/agent-policy/agent-contract.nix`에서 `agentPolicy.providers.<name>` submodule로 정의:
...
|---|---|
| Interface | `mkOption` type declarations in `agent-contract.nix` |
...
| Adapter | `agent-hook-adapters.nix` — transforms hooks to provider format |
...
Mixins (consume options, produce _hooks)
    ...
    v
agent-hook-adapters.nix (format conversion)
    |  claude: settings.json / gemini: settings.json / codex: config.toml
    v
agent-assembler.nix (assembler)
...
3. Import desired mixins (or rely on `agent-assembler.nix` which imports all)
4. Add format case to `agent-hook-adapters.nix` if needed
...
4. Add import to `lib/agent-policy/agent-assembler.nix`
