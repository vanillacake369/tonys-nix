...
`mutable-settings-sync.nix` (providing mutable config sync helpers) was moved from `lib/` to `modules/agents/mutable-settings-sync.nix` because it is an agents-only concern.
