...
{lib}: let
  ...
  spec = import ../lib/keymaps/keymap-validator.nix {inherit lib;};
  rawKeybinds = import ../lib/keymaps/keymap-binds.nix {inherit userProfile;};
  keybinds = rawKeybinds // {keymaps = spec.validate rawKeybinds.keymaps;};
  keymapPipeline = import ../lib/keymaps/keymap-pipeline.nix {inherit lib userProfile;};
  ...
  # =========================================================================
  # 9. Agent Policy Contract
  # =========================================================================
  agentPolicyTests = let
    baseHooksLib = import ../lib/agent-policy/agent-provider-hooks.nix {inherit lib;};
    hookAdapt = import ../lib/agent-policy/agent-hook-adapters.nix {inherit lib;};

    # Simulate mixin-generated hooks for adapter testing
    mockHooks = {
      "phase-gate" = {
        claude = {
          event = "PreToolUse";
          matcher = "Write|Edit";
          script = "/mock/phase-gate.sh";
          mixin = "phase-gate";
        };
      };
      "path-guard" = {
        claude = {
          ...
          script = "/mock/path-guard.sh";
          mixin = "path-guard";
        };
        gemini = {
          event = "PreToolUse";
          matcher = "";
          script = "/mock/path-guard-gemini.sh";
          mixin = "path-guard";
        };
      };
      "live-oracle" = {
        claude = {
          event = "PostToolUse";
          matcher = "Write|Edit";
          script = "/mock/live-oracle.sh";
          mixin = "live-oracle";
        };
      };
    };
    claudeAdapted = hookAdapt.claude mockHooks 5;
    geminiAdapted = hookAdapt.gemini mockHooks 5;
  in [
    # base-hooks SSoT
    (assert' "base-hooks: gemini has AfterAgent" (baseHooksLib.gemini ? AfterAgent))
    (assert' "base-hooks: codex has Stop" (baseHooksLib.codex ? Stop))
    (assert' "base-hooks: gemini notify command" (
      (builtins.head (builtins.head baseHooksLib.gemini.AfterAgent).hooks).command
      == "~/.claude/hooks/agent-notify.sh gemini"
    ))
    (assert' "base-hooks: codex notify command" (
      (builtins.head (builtins.head baseHooksLib.codex.Stop).hooks).command
      == "~/.claude/hooks/agent-notify.sh codex"
    ))

    # mergeHooks logic
    (assert' "mergeHooks: combines events" (let
      merged =
        baseHooksLib.mergeHooks
        {
          A = ["x"];
          B = ["y"];
        }
    ...
    in
      merged.A == ["x"] && merged.B == ["y" "z"] && merged.C == ["w"]))

    # hook-adapters: claude format
    (assert' "hook-adapt-claude: groups by event" (claudeAdapted ? PreToolUse && claudeAdapted ? PostToolUse))
    (assert' "hook-adapt-claude: groups by matcher" (
      builtins.length claudeAdapted.PreToolUse >= 2
    ))
    (assert' "hook-adapt-claude: has timeout" (
      (builtins.head (builtins.head claudeAdapted.PostToolUse).hooks).timeout == 5
    ))

    # hook-adapters: gemini format
    (assert' "hook-adapt-gemini: has PreToolUse" (geminiAdapted ? PreToolUse))
    (assert' "hook-adapt-gemini: grouped format" (
      builtins.length (builtins.head geminiAdapted.PreToolUse).hooks == 1
    ))

    # hook-adapters: provider isolation
    (assert' "hook-adapt: claude has no gemini hooks" (
      let
        flat = lib.flatten (lib.mapAttrsToList (
            _: entries:
              lib.flatten (map (e: map (h: h.command) e.hooks) entries)
          )
          claudeAdapted);
      in
        !builtins.any (c: lib.hasInfix "gemini" c) flat
    ))
  ];

  # =========================================================================
  # 10. Discovery & Overlays
  # =========================================================================
  discoveryTests = let
